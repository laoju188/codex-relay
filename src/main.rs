...
struct Args {
    ...
    /// Directory for JSON debug dumps. If set, each request saves:
    /// - {n}-inbound-responses.json  (Codex inbound request)
    /// - {n}-outbound-chat.json     (translated Chat Completions request)
    /// - {n}-upstream-error.json    (error body if upstream returns non-2xx)
    /// - {n}-response-events.sse     (SSE events for streaming)
    #[arg(long, env = "CODEX_RELAY_DUMP_JSON")]
...
/// Strip Authorization headers from a JSON string to avoid leaking credentials in dumps.
...
async fn handle_responses(State(state): State<AppState>, body: axum::body::Bytes) -> Response {
    let counter = DUMP_COUNTER.fetch_add(1, Ordering::SeqCst);
    ...
    handle_responses_inner(state, req, counter).await
}

async fn handle_responses_inner(state: AppState, req: ResponsesRequest, counter: u32) -> Response {
    let history = req
    ...
    if req.stream {
        ...
        handle_blocking(state, chat_req, url, model, counter).await
    }
}
...
async fn handle_blocking(
    ...
    // Debug dump: outbound Chat Completions request (auth stripped)
    if let Some(ref dir) = state.dump_json {
        let path = Path::new(dir);
        let chat_json = serde_json::to_string(&chat_req).unwrap_or_default();
        write_dump(path, "outbound-chat", counter, &strip_auth(&chat_json));
    }
    ...
    match builder.json(&chat_req).send().await {
        ...
        Ok(r) if !r.status().is_success() => {
            ...
            // Debug dump: upstream error body
            if let Some(ref dir) = state.dump_json {
                let path = Path::new(dir);
                write_dump(path, "upstream-error", counter, &body);
            }
            (
