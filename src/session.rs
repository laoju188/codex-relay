...
/// Generate a fingerprint key for an assistant turn that is more robust than
/// content-only key. Includes model, content, tool_call ids, names, and arguments
/// so that reasoning can be recovered even when Codex replays history in different
/// structures or when content hash collisions occur.
///
/// This key is stored in the turn_reasoning map alongside the content-based key,
/// providing a fallback lookup path.
pub fn assistant_turn_key(model: &str, msg: &ChatMessage) -> String {
    let mut hasher = DefaultHasher::new();
    model.hash(&mut hasher);
    msg.content.as_deref().unwrap_or("").hash(&mut hasher);
...
impl SessionStore {
    ...
    pub fn store_turn_reasoning(
        ...
        if !reasoning.is_empty() {
            ...
            // Store under assistant_turn_key (model + content + tool_calls fingerprint)
            // This is more robust than content-only key for complex replay scenarios.
            let turn_key = assistant_turn_key(model, assistant);
            self.turn_reasoning.insert(turn_key.clone(), reasoning.clone());
            debug!(
                turn_key = %turn_key,
                reasoning_len = reasoning.len(),
                "stored reasoning via assistant_turn_key"
            );
...
mod tests {
    ...
    fn test_turn_reasoning_by_content() {
        ...
        store.store_turn_reasoning("test_model", &assistant, "deep thought".into());
        assert_eq!(
    ...
    fn test_turn_reasoning_empty_content() {
        ...
        store.store_turn_reasoning("test_model", &assistant, "reason".into());
    ...
    fn test_turn_reasoning_also_stores_call_ids() {
        ...
        store.store_turn_reasoning("test_model", &assistant, "reason_tc".into());
    ...
    #[test]
    fn test_assistant_turn_key_deterministic() {
        ...
        let a = assistant_turn_key("deepseek-chat", &msg1);
        let b = assistant_turn_key("deepseek-chat", &msg1);
        assert_eq!(a, b, "same model+message should produce same key");
        ...
        let c = assistant_turn_key("deepseek-chat", &msg2);
        assert_ne!(a, c, "different call_id should produce different key");

        let d = assistant_turn_key("other-model", &msg1);
        assert_ne!(a, d, "different model should produce different key");
    }

    #[test]
    fn test_store_turn_reasoning_with_assistant_turn_key() {
        let store = SessionStore::new();
        let assistant = ChatMessage {
            role: "assistant".into(),
            content: Some("use the tool".into()),
            reasoning_content: None,
            tool_calls: Some(vec![serde_json::json!({
                "id": "call_xyz",
                "type": "function",
        ...
        store.store_turn_reasoning(
            "deepseek-v4",
            &assistant,
            "<think>important reasoning</think>".into(),
        );
        // Should be retrievable via turn_reasoning lookup
        assert_eq!(
            store.get_turn_reasoning(&[], &assistant),
            Some("<think>important reasoning</think>".into())
        );
    }
}
